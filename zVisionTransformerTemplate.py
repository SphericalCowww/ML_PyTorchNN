...
class PatchEmbedding(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # => (sampleN, embedDim, sqrt(patchN), sqrt(patchN))
        x = x.flatten(2)
        # => (sampleN, embedDim, patchN)
        x = x.transpose(1, 2)
...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        attn = (query @ key)*self.attnHeadScale         
        # => (sampleN, attnHeadN, patchN+1, patchN+1)
        attn = attn.softmax(dim=-1)
        attn = self.attnDropout(attn)
        # => (sampleN, attnHeadN, patchN+1, patchN+1)
        weight = attn @ value                           # dot product @: always the last dim of the left tensor
        # => (sampleN, attnHeadN, patchN+1, attnHeadDim)
        weight = weight.transpose(1, 2)
        # => (sampleN, patchN+1, attnHeadN, attnHeadDim) 
        weight = weight.flatten(2)
        # => (sampleN, patchN+1, inputDim)
        x = self.outputLin(weight)
...
class MultilayerPerceptron(torch.nn.Module):
    def __init__(self, inputFeatureN, hiddenFeatureN, outputFeatureN, dropoutProb=0.0):
        ...
        self.layerSquence = torch.nn.Sequential(\
            ...
            torch.nn.Linear(hiddenFeatureN, outputFeatureN),\
            torch.nn.Dropout(dropoutProb),\
        )
    def forward(self, x): 
        # input: (sampleN, patchN+1, inputFeatureN)
...
class Block(torch.nn.Module):
    def __init__(self, inputDim, attnHeadN, qkvBias=True, mlpRatio=4.0, attnDropProb=0.0, outputDropProb=0.0):
        super().__init__()
        self.norm1 = torch.nn.LayerNorm(inputDim, eps=1.0E-6)
        self.attn  = Attention(inputDim, attnHeadN, qkvBias=True, attnDropProb=attnDropProb,\
                               outputDropProb=outputDropProb)
        self.norm2 = torch.nn.LayerNorm(inputDim, eps=1.0E-6)
